...
export async function loadTemplates(ids, read = readContent) {
  ...
  return Object.fromEntries(await Promise.all(ids.map(async id => {
    ...
    const sources = template.museumStory?.sources;
    requireValue(sources === undefined || (Array.isArray(sources) && sources.every(source => {
      if (!source || !text(source.title) || !text(source.url)) return false;
      try { return new URL(source.url).protocol === 'https:'; } catch { return false; }
    })), `games/${id}.json: invalid museum sources`);
